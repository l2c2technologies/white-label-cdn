#!/bin/bash
# File: /opt/scripts/cdn/helpers/cdn-setup-letsencrypt.sh
...
# Installed to: /usr/local/bin/cdn-setup-letsencrypt (via symlink)

set -e

# Load configuration at runtime (not template substitution)
CONFIG_FILE="/etc/cdn/config.env"

if [[ ! -f "$CONFIG_FILE" ]]; then
    echo "ERROR: Configuration not found: $CONFIG_FILE" >&2
    echo "Please run: sudo cdn-initial-setup" >&2
    exit 1
fi
...
if [[ -z "$CDN_DOMAIN" ]] || [[ -z "$GITEA_DOMAIN" ]]; then
    echo "ERROR: CDN_DOMAIN and GITEA_DOMAIN must be configured" >&2
    exit 1
fi
